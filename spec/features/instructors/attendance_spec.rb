...
feature "Attendance" do
  ...
  let!(:instructor) { create_user(:first_name    => "Instructor", :last_name => "User", :github_id => "987",
                                  :role_bit_mask => 1, :cohort_id => boulder_cohort.id) }
  ...
  let!(:other_cohort_student) { create_user(:first_name => "Other", :last_name => "Cohort", :github_id => "789",
                                            :cohort_id  => denver_cohort.id, :github_username => "Student23456") }
